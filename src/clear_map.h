...
static inline uint GetFence(TileIndex t, DiagDirection side)
{
	...
	switch (side) {
		...
		case DIAGDIR_NW: return GB(GetTile(t)->m6, 2, 3);
	}
}
...
static inline void SetFence(TileIndex t, DiagDirection side, uint h)
{
	...
	switch (side) {
		...
		case DIAGDIR_NW: SB(GetTile(t)->m6, 2, 3, h); break;
	}
}
...
static inline void MakeClear(TileIndex t, ClearGround g, uint density)
{
	...
	if (!MayHaveBridgeAbove(t)) SB(GetTile(t)->m6, 6, 2, 0);
	...
	SB(GetTile(t)->m6, 2, 4, 0); // Other bits are "tropic zone" and "bridge above"
	GetTileEx(t)->m7 = 0;
}
